...
IMAGE_ROOT = "/home/azstaszewska/Data/MS Data/Split/Images/"
ANN_ROOT = "/home/azstaszewska/Data/MS Data/Split/Labels/"
...
for s in sets:
    for filename in os.listdir(IMAGE_ROOT + s + "/"):
        if "_flip" in filename or "_VRFlip" in filename or "_HRFlip" in filename or "_Gaussblur" in filename or "_saltAndPepper" in filename:
            continue

        image_path = IMAGE_ROOT + s + "/" + filename
        image = cv2.imread(image_path)
        height = image.shape[0]
        width = image.shape[1]

        ann_path = ANN_ROOT+s + "/" + filename[:-4]+".json"
        f_ann = open(ann_path, )
        ...
        for instance in annotation_json["shapes"]:
            ...
            print("First load to first ann "+str(t2-t1))
            area = 0
            new_instance = {}
            ...
            print("First ann "+str(t3-t2))

        f_ann.close()
    with open(OUT+s+"_small.json", 'w') as f_ann:  # write back to the JSON
        print(annotations)
